...
```md
[AI; hiddenFrom=ai]: Welcome! I'm a special _Unknown_ character. Your first message should <u>describe who you want me to be</u> and optionally a <u>scenario idea</u>, and I'll <a href="https://rentry.org/82hwif" target="_blank">magically</a> transform into the character you describe, and then you can chat with them.

Please reply now with your instruction or click the 'new chat' button in the top left to manually create your character(s).
```
...
window.generateCharactersAndScenario = async function (userInstruction = null) {
  ...
  try {
    ...
    oc.thread.messages.push({
      ...
      avatar: {
        url: "https://user.uploads.dev/file/f20fb9e8395310806956dca52510b16b.webp",
      },
    });
    ...
    let response = await oc.getInstructCompletion({
      instruction: [
        ...
        ``,
        `NAME: <the name of your character which is likely prominently mentioned in the instruction>`,
    ...
    if (
      ...
      !response.text.includes("TIME OF DAY")
    )
      throw new Error(`response.stopReason === "error"`);

    let text = response.text.replace(/\nTIME OF DAY.*/g, "").trim();
    let lines = text.split(/\n+/).map((l) => l.trim());
    let charName = (lines.find((l) => l.startsWith("NAME:")) || "")
      .replace("NAME:", "")
      .trim();
    let charDescription = (
      ...
      .replace("DESCRIPTION:", "")
      .trim();
    let userName = (lines.find((l) => l.startsWith("USER NAME:")) || "")
      .replace("USER NAME:", "")
      .trim();
    let userDescription = (
      lines.find((l) => l.startsWith("USER DESCRIPTION:")) || ""
    )
      .replace("USER DESCRIPTION:", "")
      .trim();
    let starter = (lines.find((l) => l.startsWith("ROLEPLAY STARTER:")) || "")
      .replace("ROLEPLAY STARTER:", "")
      .trim();
    ...
    (async function () {
      ...
      oc.character.userCharacter.avatar.url = await resizeDataURLWidth(
        dataUrl,
        300,
      );
    })();
